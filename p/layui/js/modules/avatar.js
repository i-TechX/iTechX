...
layui.define(["cropper", "jquery"], function (exports) {
    ...
    function render(options) {
        ...
        var uploadElem = options.elem || "#uploadAvatar",
            imgWidth = options.imgWidth || 500,
            ratio = options.ratio || 1,
            preview = options.preview || 'circular', // circular or square
        ...
        $('body').append([
            ...
            '                <p>图片预览：</p>',
            '                <div class="'+preview+' previewImg"></div>',
            '            </div>',
            '        </div>',
            ...
            '            <button class="l-btn sureCut" id="sureCut">确定剪裁并上传</button>',
            '        </div>',
            '    </div>',
            '</div>',
            '<!--图片裁剪框 end-->',
        ...
        //旋转
        $(".cropper-rotate-btn").off("click");
        $(".cropper-rotate-btn").on("click", function () {
            tailoringImg.cropper("rotate", 45);
        });
        //复位
        $(".cropper-reset-btn").off("click");
        $(".cropper-reset-btn").on("click", function () {
            tailoringImg.cropper("reset");
        });
        //换向
        var flagX = true;
        $(".cropper-scaleX-btn").off("click");
    ...
    layui.link('layui/css/modules/avatar/cropper.min.css');
    layui.link('layui/css/modules/avatar/ImgCropping.css');
