layui.define(['layer', 'form', 'element', 'laytpl', 'laydate', 'util', 'laypage'], function(exports){

    var Grade = {
        render: function(courseInfo){
            ...
            /* calculate answers */
            var answers = {};
            const allProbs = document.getElementsByClassName('problem');
            for (var elem_idx = 0; elem_idx < allProbs.length; elem_idx++) {
                const elem_prob = layui.$(allProbs[elem_idx]);
                for (const probElem in elem_prob.children()) {
                    if (elem_prob.children().hasOwnProperty(probElem)) {
                        if (layui.$(elem_prob.children()[probElem]).hasClass("layui-form")){
                            const elem = layui.$(elem_prob.children()[probElem])[0];
                            if (elem.hasAttribute("lay-filter")){
                                var problemID = elem.attributes['lay-filter'].nodeValue;
                                answers[problemID] = [0, {}];
                                for (const index in elem.firstElementChild.children) {
                                    if (elem.firstElementChild.children.hasOwnProperty(index)) {
                                        const questionElement = elem.firstElementChild.children[index];
                                        if (layui.$(questionElement).hasClass('question') && questionElement.hasAttribute('answer')) {
                                            const questionName = questionElement.classList[1];
                                            var type = questionElement.classList[2].replace(/q-/, '');
                                            const answer = layui.$.parseJSON(questionElement.attributes['answer'].nodeValue.replaceAll("'",'"'));

                                            var maxPoints = 0;
                                            for (const key in answer) {
                                                if (answer[key] > maxPoints) {
                                                    maxPoints = answer[key];
                                                }
                                            }
            ...
            lay('#courseHeader').html(layui.laytpl(courseHeaderTpl).render(courseInfo))
            .removeClass('layui-hide');

            var breadcrumbTpl = lay('#breadcrumb')[0].innerHTML;
            lay('#breadcrumb').html(layui.laytpl(breadcrumbTpl).render(courseInfo))
            .removeClass('layui-hide');
